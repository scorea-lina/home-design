...
export async function PATCH(
  ...
  if (body.status !== undefined) {
    ...
    if (!['todo', 'discussed', 'done', 'archived', 'resolved', 'triage', 'doing'].includes(status)) {
      return NextResponse.json({ ok: false, error: 'Invalid status' }, { status: 400 });
    }
    const normalized = status === 'archived' ? 'archived' : status === 'resolved' ? 'resolved' : status === 'done' ? 'done' : status === 'discussed' ? 'discussed' : 'todo';
